...
swift run kakaocli auth

# List your most recent chats
swift run kakaocli chats --limit 10
```
...
swift run kakaocli search "점심"

# Read recent messages from a chat (substring match on name)
swift run kakaocli messages --chat "지수" --since 7d

# Send a message (opens KakaoTalk UI automatically)
swift run kakaocli send "지수" "안녕!"

# Send to self-chat (나와의 채팅) — safe for testing
swift run kakaocli send --me _ "test message"

# Stream new messages as JSON
swift run kakaocli sync --follow

# Run a custom SQL query against the database
swift run kakaocli query "SELECT COUNT(*) FROM NTChatMessage"
```
